...
class Callback:
    ...
    def on_epoch_end(self, epoch, data, logs=None):
        pass
...
class CallbackContainer:
    ...
    def on_epoch_end(self, epoch, data, logs=None):
        logs = logs or {}
        for callback in self.callbacks:
            callback.on_epoch_end(epoch, data, logs)
...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, epoch, data, logs=None):
        ...
        if current_loss is None:
            return
        if self.best_data is np.inf:
            self.best_data = data

        loss_change = current_loss - self.best_loss

        max_improved = self.is_maximize and ((loss_change > self.tol) or ((loss_change == self.tol) and (self.best_data > data )))
        min_improved = (not self.is_maximize) and ((-loss_change > self.tol) or ((-loss_change == self.tol) and (self.best_data > data)))
